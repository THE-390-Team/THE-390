...
const CreateCommonFacilities = () => {
    ...
    const validateForm = () => {
        ...
        if (!formData.reservation_duration) {
            ...
            isValid = false;
        } else if (!/^([01]?[0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9]$/.test(formData.reservation_duration)) {
            errors.reservation_duration = "Start time must be in the format of hh:mm:ss";
            isValid = false;
        }
        ...
        if (!formData.reservation_duration_end) {
            ...
            isValid = false;
        } else if (!/^([01]?[0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9]$/.test(formData.reservation_duration_end)) {
            errors.reservation_duration_end = "End time must be in the format of hh:mm:ss";
            isValid = false;
        }
