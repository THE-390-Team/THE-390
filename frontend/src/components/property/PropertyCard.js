...
const PropertyCard = ({ property, type }) => {
  ...
  return (
    ...
    role === 'COMPANY' ? (
      <Card
        className='mb-3'
        style={{ maxWidth: '25rem', width: '100%', textAlign: 'center' }}
      >
        <Card.Img
          variant='top'
          src={property.image}
        ...
        <Card.Body>
          ...
          <Card.Title>
            <NavLink to={`/property-page/${property.id}`}>
              {property.name}
            </NavLink>
          </Card.Title>
          ...
          <Overlay
            target={unitsRef.current}
            show={showUnits}
            placement='bottom'
            rootClose={true}
            onHide={() => setShowUnits(false)}
          >
            {props => (
          ...
          <Button ref={lockersRef} onClick={toggleLockers} variant='secondary'>
            Lockers
          </Button>
          <Overlay
            target={lockersRef.current}
            show={showLockers}
            placement='bottom'
            rootClose={true}
            onHide={() => setShowLockers(false)}
          >
            {props => (
      ...
      <Card className='mb-3' style={{ width: '18rem' }}>
        <Card.Header as='h5'>
          {type} number {property.location}
        </Card.Header>
        <ListGroup variant='flush'>
          <ListGroup.Item>Location: {property.location}</ListGroup.Item>
          <ListGroup.Item>
            Purchase Price: $
            {parseFloat(property.purchase_price).toLocaleString()}
          </ListGroup.Item>
          <ListGroup.Item>
            Rent Price: ${parseFloat(property.rent_price).toLocaleString()} /
            month
          </ListGroup.Item>
          <ListGroup.Item>
            Size: {parseFloat(property.size).toFixed(2)} sqft
          </ListGroup.Item>
          ...
          {property.extra_information && (
            <ListGroup.Item>
              Extra Information: {property.extra_information}
            </ListGroup.Item>
          )}
        </ListGroup>
        <NavLink
