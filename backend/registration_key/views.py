...
from user_profile.models import User, PublicProfile, CompanyProfile
from .models import (
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            company = CompanyProfile.objects.get(
                user_id=self.request.data.get("company", None)
            )
            ...
            if user.role != "PUBLIC":
                return Response(
                    {
                        "details": "The user associated with this email is not a public user"
                    },
            ...
            serializer.save()
            key = CondoRegistrationKey.objects.get(key=serializer.data["key"])
            company.send_registration_key(key, user)
            return Response(serializer.data)

        except User.DoesNotExist:
            return Response(
                {"details": "There is no user with the given email"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        except CondoUnit.DoesNotExist:
            return Response(
                {"details": "There is no condo unit associated with the given id"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        except CompanyProfile.DoesNotExist:
            return Response(
                {"details": "There is no Company Profile associated with the given id"},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class ParkingRegistrationKeyView(ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            serializer.is_valid(raise_exception=True)
            serializer.save()
            key = ParkingRegistrationKey.objects.get(key=serializer.data["key"])
            company.send_registration_key(key, user)
            return Response(serializer.data)
            ...
            return Response(
                {"details": "There is no parking unit associated with the given id"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        except CompanyProfile.DoesNotExist:
            return Response(
...
class StorageRegistrationKeyView(ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            serializer.is_valid(raise_exception=True)
            serializer.save()
            key = StorageRegistrationKey.objects.get(key=serializer.data["key"])
            company.send_registration_key(key, user)
            return Response(serializer.data)
            ...
            return Response(
                {"details": "There is no storage unit associated with the given id"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        except CompanyProfile.DoesNotExist:
            return Response(
